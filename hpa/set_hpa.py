		...
		with open(template_file_name) as file_template:
			...
			for jj in range(len(data["spec"]["metrics"])):
				...
				if metric_resource_name == "cpu":
					data["spec"]["metrics"][jj]["resource"]["target"]["averageUtilization"] = cpu_value

				if metric_resource_name == "memory":
					data["spec"]["metrics"][jj]["resource"]["target"]["averageValue"] = memory_value

			target_file_name = "manifests/hpa-{service_name}.yaml".format(service_name=service_name)
			with open(target_file_name, 'w') as file_target:
				yaml.dump(data, file_target, sort_keys=False, default_flow_style=False)

			print(os.system("kubectl apply -f {file_name}".format(file_name=target_file_name)))
